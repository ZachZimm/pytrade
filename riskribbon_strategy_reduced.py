...
def define_risk_ribbon(_df):
    ...
    _df['riskbuycrossover2'] = np.where((_df['risk'] > _df['175rdevlower']) & (_df['risk'].shift(1) <= _df['175rdevlower'].shift(1)), _df['Close'] * 1, np.nan)
    _df['risksellcrossunder1'] = np.where((_df['risk'] < _df['29rdevupper']) & (_df['risk'].shift(1) >= _df['29rdevupper'].shift(1)), _df['Close'] * 1, np.nan)
    _df['risksellcrossunder2'] = np.where((_df['risk'] < _df['25rdevupper']) & (_df['risk'].shift(1) >= _df['25rdevupper'].shift(1)), _df['Close'] * 1, np.nan)
...
def add_indicators(_ticker, _df, _indicators):
    ...
    if(_df['risk'].isnull().all() == False):
        ...
        if(_df['riskbuycrossover1'].isnull().all() == False):
            ...
            _adps.append(mpf.make_addplot(_df['riskbuycrossover2'], color='#febf01',type="scatter", panel=0))
        if(_df['risksellcrossunder1'].isnull().all() == False):
            ...
            _adps.append(mpf.make_addplot(_df['risksellcrossunder2'], color='#adff2f',type="scatter", panel=0))
...
def generate_signals(_df, account):
    last_row = _df[-1:]
    print("Last Row: ")
    print(last_row['Close'].iloc[0])

    if (last_row['riskbuycrossover1'].iloc[0] == last_row['Close'].iloc[0]): # I don't want to have to use .iloc[-1] in the conditional, it should be part of last_row
        volume = float(account.btc_bal) * 0.5 # Try without the cast to float, it shouldn't be nessecary anymore
    ...
    if (last_row['risksellcrossunder1'].iloc[0] == last_row['Close'].iloc[0]):
